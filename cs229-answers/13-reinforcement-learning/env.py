# CS229-provided CartPole environment.
...
class CartPole:
    ...
    def simulate(self, action, state_tuple):
        """Advance (x, x_dot, theta, theta_dot) by one simulation step."""
    ...
    def get_state(self, state_tuple):
        """Discretize into 3 x 3 x 6 x 3 states, plus one failure state."""
        x, x_dot, theta, theta_dot = state_tuple
        one_deg = pi / 180
        six_deg = 6 * pi / 180
    ...
    def show_cart(self, state_tuple, pause_time):
        """Draw the current cart and pole position."""
        x, x_dot, theta, theta_dot = state_tuple
